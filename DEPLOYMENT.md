...
> ⚠️ **ВАЖЛИВО**: Запиши точні назви — вони будуть з префіксом Hostinger!
...
3. Використай FileZilla/WinSCP:
   - **Host**: ftp.ТВІЙ_ДОМЕН або IP з hPanel
...
SMTP_PASS=ТВІЙ_16_ЗНАЧНИЙ_APP_PASSWORD
...
CRYPTO_WALLET_TRC20=ТВОЯ_TRC20_АДРЕСА
CRYPTO_WALLET_BEP20=ТВОЯ_BEP20_АДРЕСА
CRYPTO_WALLET_ERC20=ТВОЯ_ERC20_АДРЕСА
CRYPTO_WALLET_BTC=ТВОЯ_BTC_АДРЕСА
...
`.htaccess` вже блокує доступ до `.env` через браузер. Перевір: відкрий `https://ДОМЕН/.env` — має бути 403 Forbidden.
...
### 6.3 Перевірка
1. Відкрий `http://ДОМЕН.com` — повинно перенаправити на `https://`
...
### Варіант B: Hostinger Email (альтернатива)
1. hPanel → **Emails** → створи email (наприклад `noreply@ДОМЕН.com`)
2. Використай налаштування:
```env
SMTP_HOST=smtp.hostinger.com
SMTP_PORT=587
SMTP_USER=noreply@ДОМЕН.com
SMTP_PASS=ПАРОЛЬ_ВІД_EMAIL
MAIL_FROM=noreply@ДОМЕН.com
```
...
2. Додай новий cron:
   ...
   - **Command**:
     ```
     /usr/bin/curl -s -H "X-Admin-Secret: ТВІЙ_ADMIN_SECRET" https://ДОМЕН.com/api/cron-verify.php > /dev/null 2>&1
     ```
3. Натисни **Створити** (Create)

> Заміни `ТВІЙ_ADMIN_SECRET` на значення `ADMIN_SECRET` з `.env`
> Заміни `ДОМЕН.com` на свій реальний домен

### 8.2 Альтернативний формат cron (PHP CLI)
Якщо curl не працює на shared hosting:
```
*/5 * * * * /usr/bin/php /home/u123456789/public_html/api/cron-verify.php --admin-secret=ТВІЙ_SECRET > /dev/null 2>&1
```
...
| 5 | config.php захищений | `https://ДОМЕН.com/api/config.php` | 403 Forbidden або пустий |
| 6 | SQL файли захищені | `https://ДОМЕН.com/schema.sql` | 403 Forbidden |
| 7 | 404 сторінка | `https://ДОМЕН.com/неіснуючий` | Кастомна 404 сторінка |
| 8 | PWA manifest | `https://ДОМЕН.com/manifest.json` | JSON manifest |
| 9 | Service Worker | `https://ДОМЕН.com/sw.js` | JavaScript файл |
