...
export function renderProfile(el){
  ...
  document.getElementById('profileForm')?.addEventListener('submit',async e=>{
    ...
    if(!ok){toast(data.message||t('profileSaveFailed'),'error');return;}
  ...
  document.getElementById('copyProfileLinkBtn')?.addEventListener('click', () => {
    ...
    if (navigator.clipboard) {
      navigator.clipboard.writeText(link).then(() => toast(t('copied'), 'success')).catch(() => toast(link, 'info'));
    } else {
      toast(link, 'info');
    }
  });
  ...
  document.getElementById('exchInitBtn')?.addEventListener('click',async()=>{
    ...
    if(step2){
      const displayCurrency=data.currency||({TRC20:'USDT',BEP20:'USDT',ERC20:'ETH',BTC:'BTC'}[network]||'USDT');
