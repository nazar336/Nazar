...
import { esc, fmtDate, toast, addNotif } from '../utils.js';
...
export function renderTasks(el){
  ...
  function filtered(){
    return (appState.S.tasks||[]).filter(task=>{
  ...
  function renderGrid(){
      ...
      return `<div class="task-card" data-tid="${task.id}">
        ...
        <div class="task-footer">
          ...
          <div style="display:flex;gap:7px;">
            ${canTake?`<button class="btn btn-primary btn-xs take-btn" data-tid="${task.id}">${t('takeTask')}</button>`:''}${!appState.isGuest&&!diffAllowed&&task.status==='open'?`<span style="font-size:11px;color:var(--warning);">🔒 ${t('lvlDiffLocked')}</span>`:''}
    ...
    // Events via delegate
    delegate(grid, 'click', '.take-btn', (e, btn) => { e.stopPropagation(); takeTask(btn.dataset.tid); });
    delegate(grid, 'click', '.complete-btn', async (e, btn) => { e.stopPropagation(); await completeTask(btn.dataset.tid, 'submit'); renderGrid(); });
    delegate(grid, 'click', '.approve-btn', async (e, btn) => { e.stopPropagation(); await completeTask(btn.dataset.tid, 'approve'); renderGrid(); });
  }
